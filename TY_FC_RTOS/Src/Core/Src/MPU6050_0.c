...
#define GYRO_SCALE       (1.0f / 16.384f)  // 陀螺仪：65.5 LSB/(°/s)（不变）
#define ACCEL_SCALE      (1.0f / 2048.0f) // 加速度计：±16g对应2048 LSB/g（关键修改）

#define COMPLEMENTARY_ALPHA 0.98f        // 权重不变，平衡平滑性和纠偏
#define SAMPLING_FREQ      100.0f        // 采样率100Hz（核心修改）
...
static HAL_StatusTypeDef MPU6050_ReadRegs(uint8_t reg, uint8_t len, uint8_t *data);
...
static HAL_StatusTypeDef MPU6050_GetRawData(short *accel, short *gyro)
{
    ...
    HAL_StatusTypeDef ret = MPU6050_ReadRegs(ACCEL_XOUT_H_REG, 14, buf);
...
void MPU6050_Calibrate(){
    ...
    MPU_Data->accel_bias[2] = accel_sum[2] / calib_count + 2048;
...
void MPU6050_GETANGLE(float dt,MPU_Data_PID_t* mpu_pid_data)
{
    ...
    ret = MPU6050_GetRawData(MPU_Data->accel, MPU_Data->gyro);
	
    // printf("raw ret=%d acc=%d %d %d gyro=%d %d %d\r\n",
    //    ret,
    //    MPU_Data->accel[0],
    //    MPU_Data->accel[1],
    //    MPU_Data->accel[2],
    ...
    mpu_pid_data->gyro_x = LowPass_Filter(MPU_Data->last_gyro_x, MPU_Data->gyro_dps[0],0.9f);
    mpu_pid_data->gyro_y = LowPass_Filter(MPU_Data->last_gyro_y, MPU_Data->gyro_dps[1],0.9f);
    mpu_pid_data->gyro_z = LowPass_Filter(MPU_Data->last_gyro_z, MPU_Data->gyro_dps[2],0.98f);
...
uint8_t mpu6050_init(void)
{
    ...
    HAL_Delay(100);
//    MPL_LOGI("1");
    MPU6050_WriteReg(SMPLRT_DIV_REG, 0x09);//设置采样率
    HAL_Delay(10);
//	MPL_LOGI("2");
    MPU6050_WriteReg(CONFIG_REG, 0x05);//低通滤波截止频率，0x04:20hz 0x03:42hz
    HAL_Delay(10);
//    MPL_LOGI("3");
    // 4. 配置陀螺仪量程：±2000°/s
    MPU6050_WriteReg(GYRO_CONFIG_REG, GYRO_FS_2000DPS << 3);//GYRO_FS_2000DPS << 3
    HAL_Delay(10);
//    MPL_LOGI("4");
    // 5. 配置加速度计量程：±16g
    MPU6050_WriteReg(ACCEL_CONFIG_REG, ACCEL_FS_16G << 3);//ACCEL_FS_16G << 3
    HAL_Delay(10);
//    MPL_LOGI("5");
	MPU6050_Calibrate();
//	MPL_LOGI("6");
